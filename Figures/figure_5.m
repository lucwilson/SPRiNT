...
% YOU MUST LOAD IN ec012data.mat BEFORE RUNNING
...
for t = 1:numel(work)
    ...
    exp = zeros(length([sd_dat.SPRiNT.channel(1).data.time]),length(sd_dat.SPRiNT.channel));
    off = zeros(length([sd_dat.SPRiNT.channel(1).data.time]),length(sd_dat.SPRiNT.channel));
    error   = ones(length(sd_dat.Time),length(sd_dat.SPRiNT.channel));
    
    for d = 1:length(sd_dat.SPRiNT.channel)
        exp(:,d) = [sd_dat.SPRiNT.channel(d).aperiodics.exponent];
        off(:,d) = [sd_dat.SPRiNT.channel(d).aperiodics.offset];
        error(:,d) = sqrt([sd_dat.SPRiNT.channel(d).stats.MSE]);
    end
    exp = mean(exp,2); % average across sensors
    off = mean(off,2); % average across sensors
    
    c = b(~isnan(movdat(1:end-1,1)));
    outstr(t).exp = exp([sd_dat.SPRiNT.channel(1).data.time]>=ceil(c(1).*2)./2 &...
        [sd_dat.SPRiNT.channel(1).data.time]<=(floor(c(end).*2)./2)); 
    outstr(t).off = off([sd_dat.SPRiNT.channel(1).data.time]>=ceil(c(1).*2)./2 &...
        [sd_dat.SPRiNT.channel(1).data.time]<=(floor(c(end).*2)./2)); 
    outstr(t).error = error([sd_dat.SPRiNT.channel(1).data.time]>=ceil(c(1).*2)./2 &...
        [sd_dat.SPRiNT.channel(1).data.time]<=(floor(c(end).*2)./2));
    ts = [sd_dat.SPRiNT.channel(1).data.time];
...
% Now focus on EC013
% YOU MUST LOAD IN ec013data.mat BEFORE RUNNING
...
imagesc(EC012_6.Time(217:337),2:0.5:40,squeeze(mean(log10(EC012_6.SPRiNT.SPRiNT_models(:,217:337,:)),1))')
set(gca,'ydir','normal');
ylim([2 40])
...
imagesc(EC013_7.Time(217:337),2:0.5:40,squeeze(mean(log10(EC013_7.SPRiNT.SPRiNT_models(:,217:337,:)),1))')
set(gca,'ydir','normal');
caxis([-4 -0.7])
colorbar;
