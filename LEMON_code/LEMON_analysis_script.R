...
###################################################################################
EEG_data=read.csv('~/Desktop/behav_LEMON/tFOOOF_Oz_api.csv', stringsAsFactors = FALSE)
MW= read.csv('Desktop/behav_LEMON/Morlet_Oz-2.csv', stringsAsFactors = FALSE)
...
data4plot = cbind(data[c(1,44)], stack(data %>% select(eo_alpha_cf_std,  eo_ap_slope_mean)))
...
data4plot = cbind(data[c(1,44)], stack(data %>% select( ec_ap_slope_mean, ec_alpha_cf_mean)))
data4plot$Age2=as.factor(data4plot$Age2)   
pd <- position_dodge(-0.2)
ggplot(data4plot, aes(x=Age2 , y = values)) + 
  stat_summary(fun.data = "mean_cl_boot", geom = "bar", size= 10, position = pd) +
...
# run regression eyes-open
lm6=glm(age ~ eo_alp_pow , data, family = 'binomial')
summary(lm6)
confint(lm6)
tab_model(lm6, transform = NULL)
tab_model(lm6)

bf = regressionBF(age ~ eo_alp_pow , MW)
plot(bf)
bf = regressionBF(age ~ eo_alp_pow , MW, whichModels = "top")
plot(bf)
summary(bf)

# run regression eyes-open
lm6=glm(age ~ ec_alp_pow , data, family = 'binomial')
summary(lm6)
confint(lm6)
tab_model(lm6, transform = NULL)


bf = regressionBF(age ~ ec_alp_pow , MW)
plot(bf)
bf = regressionBF(age ~ ec_alp_pow , MW, whichModels = "top")
plot(bf)
summary(bf)
...
data4reg = cbind(data4reg, stack(data %>% select( ec_alp_pow, eo_alp_pow)))
data4reg=data4reg[c(1,2, 4, 6, 8, 10, 12, 14)]
data4reg$cond= c(rep(0,178),rep(1,178))

colnames(data4reg)[2:8]= c("ap_slope_mean","ap_slope_std","alpha_cf_mean", "alpha_cf_std", "alpha_pow_mean", "alpha_pow_std", "MW_power")


## Logistic regression to predict condition
data2= data4reg[!is.na(data4reg$ap_slope_mean),]
data2 %>% group_by(cond) %>% summarise(m=mean(ap_slope_mean))
data2 %>% group_by(cond) %>% summarise(m=mean(alpha_pow_std))
data2 %>% group_by(cond) %>% summarise(m=mean(alpha_pow_mean))
data2 %>% group_by(cond) %>% summarise(m=mean(alpha_cf_mean))
...
data4plot = cbind(data4reg[c(1,8)], stack(data4reg %>% select( ap_slope_mean, alpha_pow_mean, alpha_pow_std)))
